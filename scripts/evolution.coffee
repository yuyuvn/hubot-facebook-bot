...
module.exports = (robot) ->
  ...
  robot.router.get "/hubot/github/evolution", (req, res) ->
    ...
    return res.send "Nothing to do" unless states.code.files?
    return res.send "Coding..." if states.code.locked

    robot.emit "run_evolution", res
    res.send "Coding..."

  robot.on "run_evolution", ->
    return if states.code.locked or not states.code.files?
    ...
    github.post("repos/#{github_config.repo}/issues",
      title: "Chuẩn bị cho nâng cấp mới #{branch_name}").then (issue) ->
    ...
    .then (ref) ->
      ...
      if ref.truncated
        ...
        for file_name, file of states.code.files
        ...
        for file, index in tree
          if states.code.files[file.path]?
            f = states.code.files[file.path]
            if f.delete
              remove.push index
            else
  ...
  robot.on "prepair_to_evolution_add_hutbot_scripts", (msg, files) ->
    ...
    if files_data["hubot-scripts.json"]?
      scripts = JSON.parse states.code.files["hubot-scripts.json"].content
      for file in files
        script = file.replace /(^scripts\/|\.coffee$)/g, ""
        scripts.push script unless script in scripts
      files_data["hubot-scripts.json"].content = "#{JSON.stringify(scripts)}\n"
    else
      sha = ""
      ...
      .then (scripts) ->
        for file in files
          script = file.replace /(^scripts\/|\.coffee$)/g, ""
          scripts.push script unless script in scripts
        states.code.set content: "#{JSON.stringify(scripts)}\n", sha: sha, "files", "hubot-scripts.json"

  robot.on "prepair_to_evolution_add_external_scripts", (msg, libs) ->
    states.code.extend files: {}
    files_data = states.code.get "files"
    if files_data["external-scripts.json"]?
      scripts = JSON.parse states.code.files["external-scripts.json"].content
      for script in libs
        scripts.push script unless script in scripts
      files_data["external-scripts.json"].content = "#{JSON.stringify(scripts)}\n"
    else
      sha = ""
      data = ref: github_config.branch
  ...
  robot.on "prepair_to_evolution_add_package", (msg, libs) ->
    ...
    if files_data["package.json"]?
      scripts = JSON.parse states.code.files["package.json"].content
