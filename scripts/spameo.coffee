...
module.exports = (robot) ->
  ...
  robot.on "room_state_handler_message_learn", (msg, state) ->
    if state.data?.code?
      ...
      msg.send "Không học nữa ạ? :'("
    room_states.set state: "learn_wait_for_finish_confirm", msg.message.room
  ...
  robot.on "room_state_handler_message_learn_wait_for_finish_confirm", (msg, state) ->
    if msg.message.text.match robot.respondPattern "^#{vocabulary_regex.yes}"
      # TODO create pull request if state.data.code is present
      robot.emit "room_state_handler_sticker_default", msg, state
    else
  ...
  robot.on "room_state_handler_sticker_learn_wait_for_condition", (msg, state) ->
    data = state.data
    condition = data.conditions.pop()
    stickerID = nsg.message.stickerID
    text = "#{vocabulary._when[0]} " # hack
    text += if condition.subject? then "#{condition.subject} " else "()"
    text += "#{condition.verb} /#{stickerID}/"
    msg.match = text.match full_sentence_regex
    robot.emit "parse_statemnet", msg, state
